...
/// Build a naked ISR stub for an exception that DOES push an error code
...
pub fn handle_gpf(frame: *TrapFrame) callconv(.c) void {
    ...
    vga.print_hex(frame.error_code);
    vga.print(" RIP=");
    vga.print_hex(frame.rip);
    vga.print("\n");
    vga.set_color(.White, .Black);
...
pub fn handle_page_fault(frame: *TrapFrame) callconv(.c) void {
    ...
    vga.print("  Faulting addr (CR2)=");
    vga.print_hex(cr2);
    vga.print("\n  Error code=");
    vga.print_hex(frame.error_code);
    // Decode error bits
    if (frame.error_code & 1 != 0) {
        vga.print(" [Protection]");
    } else {
        vga.print(" [Not-Present]");
    }
