...
pub fn load_elf(elf_data: &[u8]) -> Result<LoadedProcess, ElfError> {
    ...
    for i in 0..e_phnum {
        ...
        let phys = unsafe { crate::ffi::phys_alloc(pages) };
        if phys.is_null() {
        ...
        for page_i in 0..pages {
            ...
            let rc = unsafe { crate::ffi::page_table_map(vpage as usize, ppage, pte_flags) };
            if rc != 0 {
                kprint!("[ELF] Page mapping failed at 0x{:x}\n", vpage);
    ...
    let stack_phys   = unsafe { crate::ffi::phys_alloc(stack_pages) };
    ...
    for p in 0..stack_pages {
        ...
        unsafe { crate::ffi::page_table_map(vp as usize, pp, 0x3); } // RW, User
    }
